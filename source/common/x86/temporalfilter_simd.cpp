...
namespace X265_NS {
	...
	MCSTF_TARGET_AVX2
    // Load 4 pixels → __m256d (4 doubles)
    static inline __m256d loadPix4_pd(const pixel* p)
    {
#if X265_DEPTH > 8
        __m128i v = _mm_loadl_epi64(reinterpret_cast<const __m128i*>(p)); // 4 x uint16
        return _mm256_cvtepi32_pd(_mm_cvtepu16_epi32(v));
#else
        __m128i v = _mm_cvtsi32_si128(*reinterpret_cast<const int*>(p));  // 4 x uint8
        return _mm256_cvtepi32_pd(_mm_cvtepu8_epi32(v));
#endif
    }

    // Store 4 doubles → 4 pixels (values already rounded to integers)
	MCSTF_TARGET_AVX2
    static inline void storePix4_pd(pixel* p, __m256d v)
    {
        __m128i i32 = _mm256_cvtpd_epi32(v);   // truncate (already rounded)
#if X265_DEPTH > 8
        ...
        int blkSize, int* outVariance, int* outDiffsum)
    {
        if (blkSize == 8)
        {
            ...
            // Zero element index 7: horizontal diff is invalid at the right edge
            ...
            for (int y = 0; y < 8; y++)
            {
                ...
                // horizontal diffsum: (drow[y][x+1] − drow[y][x])², x=0..6
                ...
                // vertical diffsum: (drow[y+1][x] − drow[y][x])², y=0..6
                if (y < 7)
                {
        ...
        else // blkSize == 4
        {
...
#endif
                drow[y] = _mm_sub_epi16(s, r); // lower 4 lanes valid
            }

            // Zero elements 3..7: horizontal diff invalid at right edge (x=3)
            ...
            for (int y = 0; y < 4; y++)
            {
                __m128i d = _mm_cvtepi16_epi32(drow[y]); // 4 x int32
        ...
        pixel* dstBlk, intptr_t dstStride)
    {
        ...
        // Precompute -1/vsw[i] to avoid repeated division in the inner loop
        ...
        {
            // Process row in groups of 4 pixels
            for (int xg = 0; xg < blkSize; xg += 4)
            {
                ...
                for (int i = 0; i < numRefs; i++)
                {
                    ...
                    // newVal += weight * refVal  (separate mul+add, not FMA)
                ...
                // round(newVal): floor(x + 0.5)  — exact for x >= 0
