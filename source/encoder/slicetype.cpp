...
void CostEstimateGroup::estimatelowresmotion(MotionEstimatorTLD& m_metld, Frame* curframe, int refId)
{
    ...
    if(!(curframe->m_param->bEnableRowME))
    {
...
void CostEstimateGroup::estimatelowresmotion_doubleres(
        ...
        int                 blockRow)
{
    ...
    const int blockSize = m_mctfUnitSize; // e.g. 8
    const int stepSize  = blockSize;
    const int factor    = 1;

    const int origWidth  = orig->m_picWidth;
    const int origHeight = orig->m_picHeight;
    const int blockY     = blockRow * stepSize;

    if (blockY + blockSize > origHeight)
    ...
    // ── inner blockX loop — verbatim logic from motionEstimationLumaDoubleRes
    for (int blockX = 0; blockX + blockSize <= origWidth; blockX += stepSize)
    {
        const int blockCol = blockX / stepSize;
        ...
        if (blockRow > 0)
        {
            MV aboveMV = mvs[(blockRow - 1) * mvStride + blockCol];
            error = metld.m_useSADinME
                ? metld.motionErrorLumaSAD(metld,
                    orig->m_picOrg[0], (int)orig->m_stride,
                    buffer->m_picOrg[0], blockX, blockY,
        ...
        if (blockCol > 0)
        {
            MV leftMV = mvs[blockRow * mvStride + (blockCol - 1)];
            error = metld.m_useSADinME
                ? metld.motionErrorLumaSAD(metld,
                    orig->m_picOrg[0], (int)orig->m_stride,
                    buffer->m_picOrg[0], blockX, blockY,
        ...
        // Write result
        mvs    [blockRow * mvStride + blockCol] = best;
        minError[blockRow * mvStride + blockCol] = leastError;
    }

    // ── SIGNAL: this row is fully written ────────────────────────────────
    // Release ordering ensures all mvs[] writes above are visible to any
    // thread that observes m_rowDone[refId][blockRow] == 1.
    m_rowDone[refId][blockRow] = 1;

    // Mark the whole reference done after the last row
    if (blockRow == m_numBlockRows - 1)
        curFrame->m_lowres.lowresMcstfMvs[0][refId][0].x = 1;
}
...
void Lookahead::runMCSTF(Frame* pic)
{
   const int numRef     = pic->m_mcstf->m_numRef;
    ...
    {
        CostEstimateGroup phase1(*this, nullptr);

        for (int j = 0; j < numRef; j++)
        {
            TemporalFilterRefPicInfo* ref = &pic->m_mcstfRefList[j];

            // Skip if Phase-1 already done for this ref
            // (sentinel 0x7FFF means "never searched at all"; we only
            //  skip the very first time via the standard done-check below)
            if (pic->m_lowres.lowresMcstfMvs[0][j][0].x != 0x7FFF)
                continue;

            // add_pic: bRowMode = false, blockRow = -1
            phase1.add_pic(j,          // refIdx → stored in e.p0
                           ref->poc,   // ref POC → stored in e.p1
                           pic->m_poc, // cur POC → stored in e.b
                           pic);       // current frame
        }

        // Block until all Phase-1 jobs complete.
        // After this point ref->mvs0, mvs1, mvs2 are valid for every ref.
        phase1.finishBatch();
    }
    if(pic->m_param->bEnableRowME)
    {
        CostEstimateGroup phase2(*this, nullptr);
...
void CostEstimateGroup::add_pic(int refIdx, int poc, int curPoc, Frame* pic)
{
    // Guard: same batch-mode contract as add() / original add_row()
    X265_CHECK(m_batchMode || !m_jobTotal,
               "single CostEstimateGroup instance cannot mix batch modes\n");
    m_batchMode = true;

    Estimate& e  = m_estimates[m_jobTotal++];
    e.p0         = refIdx;   // reuse p0 as refIdx for MCSTF path
    e.p1         = poc;      // ref frame POC
    e.b          = curPoc;   // current frame POC
    e.frame      = pic;
    e.bRowMode   = false;    // ← lowres ME, ref-parallel
    e.blockRow   = -1;
