...
void Lookahead::slicetypeDecide()
{
    ...
    //We should consider bframes as 0 and remove unwanted ones
...
void Lookahead::estimateCUPropagate(Lowres **frames, double averageDuration, int p0, int p1, int b, int referenced)
{
    ...
    x265_emms();
    double fpsFactor = 1;//CLIP_DURATION((double)m_param->fpsDenom / m_param->fpsNum) / CLIP_DURATION(averageDuration);
...
{
    int fpsFactor = 1;//(int)(CLIP_DURATION(averageDuration) / CLIP_DURATION((double)m_param->fpsDenom / m_param->fpsNum) * 256);
